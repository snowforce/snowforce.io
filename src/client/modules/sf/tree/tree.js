...
export default class sfTree extends LightningElement {
  ...
  syncSelected() {
    if (this.treedata && this._childNodes.length > 0) {
      this._selectedItem = this.treedata.syncSelectedToData(this.selectedItem);
  ...
  updateCurrentFocusedChild() {
    if (this._key === this._currentFocusedItem.parent) {
      ...
      this.treedata.updateCurrentFocusedChild(this._currentFocusedItem.key);
    }
  }
  ...
  renderedCallback() {
    ...
    if (this.hasDetachedListeners) {
      const container = this.template.querySelector('.slds-tree_container');

      container.addEventListener('focus', this.handleTreeFocusIn.bind(this));
  ...
  expandBranch(node) {
    if (!node.isLeaf && !node.isDisabled) {
      ...
      if (this._selectedItem && this._selectedItem.key.startsWith(node.key)) {
  ...
  setFocusToItem(item, shouldFocus = true) {
    ...
    if (item) {
      ...
      if (this.callbackMap[item.parent]) {
        this.callbackMap[item.parent].focusCallback(item.key, shouldFocus);
      }
    }
  }
